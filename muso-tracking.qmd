...
## Nombre de bénéficiaires par groupe
::: card
```{python, echo=FALSE}
plot_beneficiaries_by_categorie(
  df = muso_ben,
...
df_sum <- household_number_r %>%
  group_by(last_modified_by_user_username) %>%
...
ggplot(df_sum, aes(x = reorder(last_modified_by_user_username, total_households), 
                   y = total_households, 
                   fill = last_modified_by_user_username)) +
  geom_col() +
...
ben_sans_ppi = muso_ben[~muso_ben["caseid"].isin
plot_beneficiaries_by_categorie(
  df = ben_avec_ppi,
