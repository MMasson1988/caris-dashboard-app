...
## Participation en club des femmes enceintes pour cette semaine
```{r, echo=FALSE}
...
prev <- floor_date(d - 0, unit = "week", week_start = 1)
...
```{r club_activity2, echo=FALSE}

#start_date_club = '2025-01-01'
#end_date_club = '2025-07-30'

# Fixer la date de fin manuellement
start_date_club <- as.Date("2025-01-01")

# Fixer la date de début à aujourd'hui
...
club_s_ptme$graduation_date[is.na(club_s_ptme$graduation_date)] <- as.Date("0000-00-00", format = "%Y-%m-%d")
...
#graf_abs_pres_ptme
write_xlsx(graf_abs_pres_ptme, path = "graf_abs_pres_ptme.xlsx")
```
...
## Représentation graphique de l'effectif total des femmes PTME attendues en club pour la semaine écoulée  versus celles qui y ont effectivement participé
```{r club2, echo=FALSE}
library(dplyr)
library(ggplot2)
library(lubridate)

# Définir les bornes temporelles : semaine écoulée (lundi → dimanche)
today <- Sys.Date()
last_monday <- today - wday(today) - 5  # Lundi dernier (semaine écoulée)
last_sunday <- last_monday + 6          # Dimanche suivant

clb_ptme_df <- club_s_ptme %>%
  select(patient_code, hopital, dob, sex, club_name, club_type,
         `session_date(presence)`, last_attendance_date, 
         is_abandoned, is_dead, graduation, raison_absence) %>%
  filter((`session_date(presence)` >= last_monday & `session_date(presence)` <= last_sunday) &
           club_type == 'Club Mere' & is_abandoned != 1 & is_dead != 1 & graduation == 'Non') %>%
  mutate(
    semaine = paste0("Semaine du ", format(last_monday, "%d %b"), " au ", format(last_sunday, "%d %b")),
    site_code = substr(patient_code, 0, 8)
  )

if(nrow(clb_ptme_df) == 0){
  print("Il n'y a pas d'activité pour la semaine écoulée.")
} else {
  total_ptme <- length(unique(clb_ptme_df$patient_code))
  part_ptme <- clb_ptme_df %>%
    filter(raison_absence == '1- Present') %>%
    summarise(participation = n_distinct(patient_code)) %>%
    pull(participation)

  df_clb_ptme <- data.frame(var = c("Effectif", "Participation"),
                            freq = c(total_ptme, part_ptme))

  df_clb_ptme %>%
    ggplot(aes(y = freq, x = reorder(var, freq))) + 
   gplot(aes(y = freq, x = reorder(var, freq))) + 
    geom_bar(fill = "#00AFBB", stat = "identity", color = "black", show.legend = FALSE) +
    geom_label(aes(label = freq, hjust = 0.5), size = 3.3, show.legend = FALSE) +
    coord_flip() +
    theme_bw() +
...
:::card
## Représentation graphique de l'effectif total des femmes PTME attendues en club pour le bimestre en cours versus celles qui y ont effectivement participé

```{r club3, message=FALSE, warning=FALSE}

library(dplyr)
library(lubridate)
library(ggplot2)

# Conversion sûre de la date si nécessaire
club_s_ptme$`session_date(presence)` <- as.Date(club_s_ptme$`session_date(presence)`)

# Définir la période d'analyse
end_date <- Sys.Date()
start_date <- floor_date(end_date %m-% months(2), unit = "month")

# Filtrage des données PTME Club Mère
clb_ptme_df <- club_s_ptme %>%
  select(
  ...
  mutate(
    mois = paste(
      lubridate::month(`session_date(presence)`, label = TRUE, abbr = FALSE),
      lubridate::year(`session_date(presence)`)
    ),
    site_code = substr(patient_code, 1, 8)
  )

# Vérification : y a-t-il des données à afficher ?
if (nrow(clb_ptme_df) == 0) {
  cat("**Il n'y a pas d'activité pour la période sélectionnée.**")
} else {
  total_ptme <- n_distinct(clb_ptme_df$patient_code)

  part_ptme <- clb_ptme_df %>%
    filter(raison_absence == "1- Present") %>%
    summarise(participants = n_distinct(patient_code)) %>%
    pull(participants)

  df_clb_ptme_mois <- tibble(
  ...
  df_clb_ptme_mois %>%
    ...
    geom_label(aes(label = freq), size = 3.5) +
    coord_flip() +
    theme_bw() +
    labs(caption = paste("Data source: hivhaiti /", Sys.Date())) +
    theme(
...
# Calculer la date de début : deux mois avant, au premier jour du mois
start_date <- floor_date(end_date %m-% months(2), unit = "month")
...
clb_ptme_df <- club_s_ptme %>%
  ...
  mutate(
    mois = format(`session_date(presence)`, "%Y-%m")
  )
...
## Données décrivant les activités dans les clubs pour la  semaine 
...
first_part <- "C:\\Users\\moise\\Downloads\\"
...
## Représentation graphique des fréquences de femmes participant en club en Q3 ou Q4 FY24 avec un comptage de ménage ou non
...
## Liste dynamique des femmes PTME ayant au moins une (1) participation en club en Q2 ou Q3 FY24 sans comptage de ménage
...
comptage_ptme_no_0<-comm_ptme_mix%>%
  distinct(patient_code, .keep_all = TRUE)%>%
  filter(on_commCare == 'no' & !patient_code %in% accept$patient_code)%>%
  mutate(site_code = substr(patient_code,0,8))

comptage_ptme_no = left_join(comptage_ptme_no_0, club_infos%>%select(-c(office)), by='patient_code')
...
# STATISTIQUES DE PARTICIPATION DES FEMMES PTME EN CLUB EN Q3 & EN Q4 FY24


## Représentation graphique des statistiques de participation en club en Q3 & Q4 FY24
```{r,echo=FALSE}
...
## Liste dynamique des femmes PTME ayant eu au moins une (1) participation en club en Q3 mais aucune en Q4 FY24 (jusqu'ici)
...
# STATISTIQUES DES SERVICES AUX FEMMES PTME EN Q3 & EN Q4 FY24


## Représentation graphique des statistiques sur les femmes servies en Q3 & Q4 FY24
```{r, echo=FALSE}
library(dplyr)
library(lubridate)
# IMPORT PTME appel
...
## Liste dynamique des femmes PTME ayant bénéficié d'au moins un (1) service en Q3 mais aucun en Q4 FY24 (jusqu'ici)
...
# ACTIVITE DANS LES CLUBS EN Q4 FY24


## Représentation graphique de l'effectif total des femmes PTME attendues en club en Q4 FY24  versus celles qui y ont effectivement participé
...
## Représentation graphique des raisons d'absence en club en Q4 FY24 
...
## Tableau des données décrivant les activités dans les clubs en Q4 FY24 
...
cr_club <- cr_club%>%
  select(club_name, club_type, start_date, last_session_date, Quarter) %>%
  filter(club_type == 'Club Mere')
...
office<-cr_club%>%
  select(club_name, Quarter)%>%
	group_by(club_name, Quarter) %>%
  count() %>%
  ungroup()
...
office %>%
	group_by(club_name)%>%
	mutate(prop=n/sum(n))%>%
	ggplot(aes(fill=Quarter, y=n, x=reorder(club_name,n)))+
...
## Figure représentative des néophytes intégrées en club en Q4 FY24
...
## Liste des néophytes intégrées en club en Q4 FY24
...
# STATISTIQUES RELATIVES AUX PCR NON LIES EN FY24
...
## Table des PCRs non liés en FY24
...
```{r delivery without pcr, echo=FALSE, results='asis'}

#Reported for the entire fiscal year
start_date_dwp = '2025-01-01'
end_date_dwp = '2025-07-10'
...
#### Figure 9.2 - Nombre d’enfants exposés ayant un test PCR par tranche d’âge en mois pour `r paste(period,sep="_","FY25")`: mères en club vs mères non en club vs mères non sur hivhaiti
