...
def plot_beneficiaries_by_categorie(df, lo_department, set_title, set_xlabel, set_ylabel):
    ...
    # Titre et labels
    ax.set_title(f"{set_title}\nTotal bénéficiaires : {total_active_patients}", fontsize=22, weight='bold')
...
```{r club_activity, echo=FALSE}

start_date_club = '2025-03-30'
end_date_club = '2025-06-30'
...
# Obtenir la date du lundi de la semaine précédente (20 jours avant, puis arrondi à la semaine)
prev <- floor_date(d - 1, unit = "week", week_start = 1)

# Obtenir une date 5 jours après 'prev' (par exemple, vendredi de cette semaine-là)
after <- prev + days(4)
...
clb_ptme_df <- club_s_ptme %>%
  select(patient_code, hopital, dob, sex, club_name, club_type,
         `session_date(presence)`, last_attendance_date, 
         is_abandoned, is_dead, graduation, raison_absence) %>%
  filter((last_attendance_date >= start_date & last_attendance_date <= end_date) &
           club_type == 'Club Mere' & is_abandoned != 1 & is_dead != 1 & graduation == 'Non') %>%
  mutate(
    mois = paste(
      lubridate::month(`session_date(presence)`, label = TRUE, abbr = FALSE),
      lubridate::year(last_attendance_date)
    ),
    site_code = substr(patient_code, 0, 8)
  )
...
if(nrow(clb_ptme_df) == 0){
  ...
  part_ptme <- nrow(part_ptme_x)

  df_clb_ptme_mois <- data.frame(var = c("Effectif", "Participation"),
                            freq = c(total_ptme, part_ptme))
...
# combiner data_infi_club et serv_ptme (femme_enceinte)
...
# Obtenir la date du lundi de la semaine précédente (10 jours avant, puis arrondi à la semaine)
prev <- floor_date(d - 1, unit = "week", week_start = 1)
...
```
```{r club_activity2, message=FALSE}
library(DBI)
library(RMySQL)
library(lubridate)
...
start_date_club <- end_date_club %m-% months(6)

# Connexion et requête avec substitution des dates
sess_club <- dbSendQuery(
  dbConnect(
    MySQL(), 
    user = 'caris_readonly', 
  ...
  gsubfn::fn$identity(sprintf("
    ...
    FROM session ss
      ...
      LEFT JOIN (
        ...
        FROM session s1
          LEFT JOIN club_session cs1 ON cs1.id = s1.id_club_session
          LEFT JOIN (SELECT p12.*, IF(p12.linked_to_id_patient>0, p12.linked_to_id_patient, p12.id) AS main_id FROM patient p12) pp 
            ON pp.id = s1.id_patient
    ...
    WHERE
      ...
      cs.date BETWEEN '%s' AND '%s'
    ORDER BY CONCAT(lh.city_code, '/', lh.hospital_code), c.name, p.patient_code, cs.date
    LIMIT 100000000
  ", start_date_club, end_date_club))
)

# Récupération
club_s_ptme <- fetch(sess_club, n = -1)
head(club_s_ptme, 3)
writexl::write_xlsx(club_s_ptme, 'club_s_ptme_club.xlsx')
```
...
club_df_ptme <- club_s_ptme %>%
  ...
  filter(
    ...
    is_abandoned != 1,
    is_dead != 1,
    last_attendance_date >= prev,
    last_attendance_date <= after
  )
...
clb_ptme_df <- club_s_ptme %>%
  ...
  filter(
    last_attendance_date >= start_date & last_attendance_date <= end_date,
    club_type == 'Club Mere',
    is_abandoned != 1,
    is_dead != 1,
    graduation == 'Non'
